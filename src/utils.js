...
export const filterByCriterias = (journalEntries, userInput, accounts) => {
  ...
  const startAcc = (startAccount !== undefined && startAccount !== null) ? startAccount : -99999999;
  const endAcc = (endAccount !== undefined && endAccount !== null) ? endAccount : 99999999;

  const startPer = (startPeriod && startPeriod.getTime) ? startPeriod : new Date(-8640000000000000);
  const endPer = (endPeriod && endPeriod.getTime) ? endPeriod : new Date(8640000000000000);

  const accountMap = new Map(accounts.map(account => [account.ACCOUNT, account]));
  const balanceMap = new Map();

  journalEntries.forEach(({ ACCOUNT, DEBIT, CREDIT, PERIOD }) => {
    if (ACCOUNT < startAcc || ACCOUNT > endAcc) return;
    if (PERIOD < startPer || PERIOD > endPer) return;

    const account = accountMap.get(ACCOUNT);
    if (!account) return;

    if (balanceMap.has(ACCOUNT)) {
      const entry = balanceMap.get(ACCOUNT);
      entry.DEBIT += DEBIT;
      entry.CREDIT += CREDIT;
